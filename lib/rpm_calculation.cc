...
uint32_t raw_to_millihertz(const std::vector<uint8_t>& readings, uint16_t sample_rate_hz) {
  // TODO saturation!
...
uint32_t raw_to_millirpm(const std::vector<uint8_t>& readings, uint16_t sample_rate_hz, uint16_t sensor_count) {
    return raw_to_millihertz (readings, sample_rate_hz) * util::HERTZ_TO_RPM / sensor_count;
}
