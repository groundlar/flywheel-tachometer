...
TEST(RpmTest, raw_to_millihertz_empty) {
    ...
    for (int16_t rate : sample_rates) {
        for (int16_t sensors: sensor_counts) {
          int32_t result = lib_rpm_calculation::raw_to_millihertz(
              empty_readings, rate);
          EXPECT_EQ(result, 0);
          result = lib_rpm_calculation::raw_to_millirpm(
...
TEST(RpmTest, raw_to_millihertz_alternating_single) {
    ...
    uint32_t result = lib_rpm_calculation::raw_to_millihertz(readings, sample_rate);
...
TEST(RpmTest, raw_to_millihertz_alternating_sample) {
    ...
    for (auto sample_rate : sample_rates) {
        uint32_t expected = util::MILLIS * sample_rate * oscillations / readings.size();
        uint32_t result = lib_rpm_calculation::raw_to_millihertz(readings, sample_rate);
        EXPECT_EQ(result, expected);
    }
}
